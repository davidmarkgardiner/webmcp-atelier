...
description: >
  ...
  display + Inter body + SFMono-Regular for the index/code voice on a warm-navy code surface,
...
typography:
  ...
  kicker:  { fontFamily: "SFMono-Regular", px: 28, cqw: 1.46, weight: 500, tracking: "0.16em", upper: true }
  mono-label:{ fontFamily: "SFMono-Regular", px: 26, cqw: 1.35, weight: 500, tracking: "0.02em" }
  code:    { fontFamily: "SFMono-Regular", cqw: 1.67, weight: 400, lineHeight: 1.6 }
  ...
  number-unit:{ fontFamily: "SFMono-Regular", cqw: 2.08, weight: 500, lineHeight: 1.0 }
...
components:
  ...
  kicker-spike:
    ...
    description: "The eyebrow — SFMono-Regular uppercase, indexical (2–5 words), prefixed with the coral ✱. Never plain text, never a sentence."
  coral-callout:
  ...
  number-lockup:
    ...
    description: "Hero stat — a Inter figure paired with a SFMono-Regular unit (200K, +1,204, −318, 17 files). Figure is serif; the unit is ALWAYS mono, never the serif."
  pull-quote:
  ...
  code-surface:
    ...
    textColor: "{colors.cream} (SFMono-Regular); syntax in coral (keywords) / teal #5DB8A6 (strings) / amber #E8A55A (numbers)"
...
- **Weights** — the brand font ships `{400, 500, 600, 700, 750, 800}` only; every weight is clamped to these — ignore higher preset weights (e.g. 600/700) in prose.
...
UI chrome. **SFMono-Regular** carries the indexical layer — kickers, technical labels, the code
...
- **Inter** (sentence case, negative-tracked) for all display; **Inter** body/chrome; **SFMono-Regular** index + code.
...
- **Type** — Inter sentence-case display (negative-tracked); Inter 400 body; SFMono-Regular kickers (uppercase 0.16em, coral ✱) + code.
...
- Set all display in **Inter, sentence case**, negative-tracked; **Inter 400** body; **SFMono-Regular** kickers (uppercase, 0.16em, coral ✱) + code.
...
- **Type** — Inter sentence-case display (negative-tracked); Inter 400 body; SFMono-Regular kickers (uppercase 0.16em, coral ✱) + code; ≥1.4cqw floor.
