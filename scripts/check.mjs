...
for (const candidate of manifest.candidates) {
  ...
  if (
    candidate.dimensions.join(",") !==
    "webmcpLeverage,execution,impact,creativity"
  ) {
    throw new Error(
      `candidate ${candidate.id} must use the official four judging dimensions`,
    );
  }
...
console.log(
  `ATELIER_CHECK_OK candidates=${manifest.candidates.length} submission_limit=${manifest.submissionLimit}`,
);
